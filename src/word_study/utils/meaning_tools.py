...
v2 变更:
  - create_meaning_node 将 CREATE 改为 MERGE，避免绕过管线时产生重复节点
  - 添加结构化 logger
"""
...
def create_meaning_node(word_name: str, pos_label: str, meaning: str,
                        plot: str = "", plot_embedding: list[float] | None = None,
                        example_sentences: list[str] | None = None) -> list[dict]:
    """创建释义节点 + TRANSLATION_INTO 关系，mean_query_times 初始 1。 v2: MERGE 幂等.

    节点同时带有 :Meaning 标签 + plot / plotEmbedding 用于向量搜索。 MERGE 确保绕过管线也能安全调用。
    v3: 支持 example_sentences 例句字段。
    """
    graph = get_graph()
    ...
    MERGE (m:{label}:Meaning {{means: [$meaning]}})
    ON CREATE SET m.plot = $plot, m.plotEmbedding = $plot_embedding, m.example_sentences = $example_sentences
    ON MATCH SET
        m.plot = CASE WHEN $plot <> '' AND (m.plot IS NULL OR m.plot = '') THEN $plot ELSE m.plot END,
        m.plotEmbedding = CASE WHEN size($plot_embedding) > 0 AND (m.plotEmbedding IS NULL OR size(m.plotEmbedding) = 0) THEN $plot_embedding ELSE m.plotEmbedding END,
        m.example_sentences = CASE WHEN size($example_sentences) > 0 AND (m.example_sentences IS NULL OR size(m.example_sentences) = 0) THEN $example_sentences ELSE m.example_sentences END
    MERGE (w)-[t:TRANSLATION_INTO]->(m)
    ON CREATE SET t.mean_query_times = 1
    ON MATCH SET t.mean_query_times = t.mean_query_times + 1
    RETURN m.means AS means, t.mean_query_times AS mean_query_times, labels(m) AS pos, m.example_sentences AS example_sentences
    """
    params = {
